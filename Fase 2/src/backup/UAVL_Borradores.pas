...
var SL: TStringList;
  ...
  begin
    if N = nil then Exit;
    lbl := Format('ID=%d | %s | bf=%d',
                  [N^.Key, StringReplace(N^.Data.Asunto,'"','''',[rfReplaceAll]), BalanceFactor(N)]);
    SL.Add(Format('  "n%d" [label="%s"];', [N^.Key, lbl]));
    if N^.L <> nil then
    begin
...
begin
  ...
  try
    ...
    SL.Add('  node [shape=record, style=filled, fillcolor="#E8F5E9"];');
    SL.Add('  rankdir=TB;');
    if Root = nil then
