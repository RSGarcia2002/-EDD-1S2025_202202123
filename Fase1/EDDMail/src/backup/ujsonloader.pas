...
begin
  ...
  try
    ...
    begin
      ...
      id      := JsonToString(obj.Find('id')).Trim;
      nombre  := obj.Get('nombre','').Trim;
      usuario := obj.Get('usuario','').Trim;
      email   := obj.Get('email','').Trim;
      tel     := obj.Get('telefono','').Trim;
      passw   := obj.Get('password','').Trim; // opcional
