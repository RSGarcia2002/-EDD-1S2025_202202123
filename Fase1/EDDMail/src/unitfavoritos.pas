...
procedure TFormFavoritos.VisitAdd(const M: TMailFav);
var
  idx: Integer;
begin
  // Guardar copia en arreglo
  idx := Length(FItems);
  SetLength(FItems, idx + 1);
  FItems[idx] := M;

  // Mostrar en lista
  lbFavs.Items.AddObject(
    Format('ID=%d | %s -> %s | %s', [M.Id, M.Remitente, M.Destinatario, M.Asunto]),
    TObject(PtrInt(idx))   // guardamos el índice para recuperar el detalle
  );
end;
